...
export class AuditService {
  private readonly repo: JsonRepository<AuditEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'audit');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["action","entity","userName"],
      filters,
    });
  }
  ...
  async remove(id: string) {
    ...
    if (!ok) throw new NotFoundException('Audit not found');
    return { deleted: true };
  }
