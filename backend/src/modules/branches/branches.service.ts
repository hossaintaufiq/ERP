...
export class BranchesService {
  private readonly repo: JsonRepository<BranchesEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'branches');
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Branches not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Branches not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Branches not found');
    return { deleted: true };
  }
