...
export class WarehousesService {
  private readonly repo: JsonRepository<WarehousesEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'warehouses');
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Warehouses not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Warehouses not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Warehouses not found');
    return { deleted: true };
  }
