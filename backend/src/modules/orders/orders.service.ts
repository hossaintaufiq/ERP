...
export class OrdersService {
  private readonly repo: JsonRepository<OrdersEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'orders');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["orderNumber","buyer","styleNumber"],
      filters,
    });
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Orders not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Orders not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Orders not found');
    return { deleted: true };
  }
