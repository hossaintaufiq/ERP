...
export class PayrollService {
  private readonly repo: JsonRepository<PayrollEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'payroll');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["employeeName","employeeCode","period"],
      filters,
    });
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Payroll not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Payroll not found');
    return { deleted: true };
  }
