...
export class SuppliersService {
  private readonly repo: JsonRepository<SuppliersEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'suppliers');
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Suppliers not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Suppliers not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Suppliers not found');
    return { deleted: true };
  }
