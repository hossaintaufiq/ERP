...
export class NotificationsService {
  private readonly repo: JsonRepository<NotificationsEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'notifications');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["title","type","severity"],
      filters,
    });
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Notifications not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Notifications not found');
    return { deleted: true };
  }
