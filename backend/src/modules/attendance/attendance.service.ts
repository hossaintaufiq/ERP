...
export class AttendanceService {
  private readonly repo: JsonRepository<AttendanceEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'attendance');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["employeeName","employeeCode","status"],
      filters,
    });
  }
  ...
  async remove(id: string) {
    ...
    if (!ok) throw new NotFoundException('Attendance not found');
    return { deleted: true };
  }
