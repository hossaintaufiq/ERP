...
export class EmployeesService {
  private readonly repo: JsonRepository<EmployeesEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'employees');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["name","employeeCode","department"],
      filters,
    });
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Employees not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Employees not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Employees not found');
    return { deleted: true };
  }
