...
export class StylesService {
  private readonly repo: JsonRepository<StylesEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'styles');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["name","styleNumber","buyer"],
      filters,
    });
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Styles not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Styles not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Styles not found');
    return { deleted: true };
  }
