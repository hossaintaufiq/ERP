...
export class LeadsService {
  private readonly repo: JsonRepository<LeadsEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'leads');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["companyName","contact","status"],
      filters,
    });
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Leads not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Leads not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Leads not found');
    return { deleted: true };
  }
