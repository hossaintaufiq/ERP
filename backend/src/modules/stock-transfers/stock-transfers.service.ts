...
import { StockTransfersEntity } from './stock-transfers.entity';

@Injectable()
export class StockTransfersService {
  private readonly repo: JsonRepository<StockTransfersEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'stock-transfers');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["transferNumber","status"],
      filters,
    });
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('StockTransfers not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('StockTransfers not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('StockTransfers not found');
    return { deleted: true };
  }
