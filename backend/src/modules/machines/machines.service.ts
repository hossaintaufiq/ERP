...
import { MachinesEntity } from './machines.entity';

@Injectable()
export class MachinesService {
  private readonly repo: JsonRepository<MachinesEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'machines');
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Machines not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Machines not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Machines not found');
    return { deleted: true };
  }
