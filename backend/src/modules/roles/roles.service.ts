...
export class RolesService {
  private readonly repo: JsonRepository<RolesEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'roles');
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Roles not found');
    return item;
  }
  ...
  async remove(id: string) {
    ...
    if (!ok) throw new NotFoundException('Roles not found');
    return { deleted: true };
  }
