...
import { QuotationsEntity } from './quotations.entity';

@Injectable()
export class QuotationsService {
  private readonly repo: JsonRepository<QuotationsEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'quotations');
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Quotations not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Quotations not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Quotations not found');
    return { deleted: true };
  }
