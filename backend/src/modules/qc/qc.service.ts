...
import { QcEntity } from './qc.entity';

@Injectable()
export class QcService {
  private readonly repo: JsonRepository<QcEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'qc');
  }
  ...
  findAll(query: PaginationQueryDto & Record<string, any>) {
    ...
    return this.repo.findAll({
      ...
      sortBy,
      sortDir,
      searchFields: ["orderNumber","gate","result"],
      filters,
    });
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Qc not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Qc not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Qc not found');
    return { deleted: true };
  }
