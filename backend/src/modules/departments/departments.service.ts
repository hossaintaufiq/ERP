...
export class DepartmentsService {
  private readonly repo: JsonRepository<DepartmentsEntity>;

  constructor(store: JsonFileStore) {
    this.repo = new JsonRepository(store, 'departments');
  }
  ...
  async findOne(id: string) {
    ...
    if (!item) throw new NotFoundException('Departments not found');
    return item;
  }
  ...
  async update(id: string, dto: Record<string, any>) {
    ...
    if (!updated) throw new NotFoundException('Departments not found');
    return updated;
  }

  async remove(id: string) {
    const ok = await this.repo.delete(id);
    if (!ok) throw new NotFoundException('Departments not found');
    return { deleted: true };
  }
